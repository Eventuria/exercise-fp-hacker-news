  ...
  case class TopCommenterStory (
    ...
    override def toString: String = {
      val toStringTopCommenter :String = topCommenter match {
        case topCommenter if topCommenter.size == 0  => "No Commenter yet\n"
        case topCommenter => Monoid.combineAll[String](
            topCommenter.
              ...
              map {case ((nbComments,users),index) => {
                ...
                s"\t${"%3d".format(index+1)}  - $nbComments comment(s) - ${userList} \n"}})
      }
...
object TopCommenterApp extends IOApp {
  ...
  class HackerNewsClient(httpClient : Client[IO]) {
    ...
    def streamComments (commentId: CommentId) : Stream [IO, Comment] = {
        Stream
          .emit[IO,CommentId](commentId)
          .evalMap[IO,Comment](commentId => httpClient.expect[Comment](baseUrl / "v0" / "item" / s"$commentId.json"))
          .flatMap {
            case comment @IdentifiedComment(_, _, comments) =>
              Stream
                .emit[IO,Comment](comment)
                .merge(Stream.emits[IO,CommentId](comments.toList).flatMap(streamComments))

            // Recursion Base Cases
            case comment @AnonymousComment(_, _)            => Stream.emit[IO,Comment](comment)
            case comment @UncommentedIdentifiedComment(_,_) => Stream.emit[IO,Comment](comment)
            case comment @UncommentedAnonymousComment(_)    => Stream.emit[IO,Comment](comment)
          }
        ...
        story match {
          ...
          case CommentedStory(id, title, comments) =>
            Stream.emits[IO,CommentId](comments.toList)
              ...
              .compile
              .fold(TopCommenterStoryBuilder(id, title))( (builder,newComment) => newComment match {
